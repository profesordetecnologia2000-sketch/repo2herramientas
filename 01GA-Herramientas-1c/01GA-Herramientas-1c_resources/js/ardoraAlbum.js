...
if ((typeGame==0) || (typeGame==1)){successes=0;
  for (i=0; i <4; i++){
    for (j=0; j <4; j++){
      ...
      if ($.trim(answers[z][t]).replace("&#39;","'") !=respu){correct=false;}else{successes++;$("#txt" + (i + 1).toString() + (j + 1).toString()).removeClass("ui-droppable");$("#txt" + (i + 1).toString() + (j + 1).toString()).removeClass("txtCell_txt");$("#"+nameDiv).removeClass("ui-draggable");$("#"+nameDiv).removeClass("ui-draggable-handle");$("#" + nameDiv).css({"pointer-events":"none"});$("#"+nameDiv).removeClass("txtCell_txt");$("#"+nameDiv).css("font-weight","bold");}}}}}}}}
...
$.trim(alt4[z][t]).replace("&#39;", "'") != respu && $.trim(alt5[z][t]).replace("&#39;", "'") != respu || respu.length==0){ correct = false;} else { successes++;$("#"+nameDiv).prop("readonly", true);$("#"+nameDiv).css("font-weight","bold");}}}}}}}}}
      ...
      var respu=$.trim(document.getElementById(nameDiv).innerHTML.replace("<p>","").replace("</p>",""));
      if ((board[i][j]!=$(cel).attr("src")) || ($.trim(answers[i][j]).replace("&#39;","'") !=respu)){correct=false;}else{successes++;$("#txt" +(i+1).toString()+(j+1).toString()).removeClass("ui-droppable");$("#txt"+(i+1).toString()+(j+1).toString()).removeClass("txtCell_txt");$("#"+nameDiv).removeClass("ui-draggable");$("#"+nameDiv).removeClass("ui-draggable-handle");$("#"+nameDiv).removeClass("txtCell_txt");$("#"+nameDiv).css("font-weight", "bold");$("#ima"+(i+1).toString()+(j+1).toString()+"_img").removeClass("ui-droppable");$("#ima"+(i+1).toString()+(j+1).toString()+"_img img").removeClass("ui-draggable");$("#ima"+(i+1).toString()+(j+1).toString()+"_img img").removeClass("ui-draggable-handle");$("#ima" + (i + 1).toString() + (j + 1).toString() + "_img img").css({"pointer-events":"none"});}}}}}
